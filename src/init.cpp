...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Astrum.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Astrumd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeAstrumamount=<n> " + _("Keep N Astrum anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Astrum is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Astrum is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Astrum version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Astrum server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Astrum") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Astrum to complete") << "\n";
    ...
    nAnonymizeAstrumAmount = GetArg("-anonymizeAstrumamount", 0);
    if(nAnonymizeAstrumAmount > 999999) nAnonymizeAstrumAmount = 999999;
    if(nAnonymizeAstrumAmount < 2) nAnonymizeAstrumAmount = 2;
    ...
    LogPrintf("Anonymize Astrum Amount %d\n", nAnonymizeAstrumAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1ASTR+1000 == (.1ASTR+100)*10
       10ASTR+10000 == (1ASTR+1000)*10
    */
