...
int nAnonymizeAstrumAmount = 1000;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Astrum
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Astrum
    // Mac: ~/Library/Application Support/Astrum
    // Unix: ~/.Astrum
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Astrum";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Astrum.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "Astrumd.pid"));
