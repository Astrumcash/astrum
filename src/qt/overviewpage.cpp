...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeAstrumAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeAstrumAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeAstrumAmount = strAnonymizeAstrumAmount.remove(strAnonymizeAstrumAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeAstrumAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeAstrumAmount*COIN) nMaxToAnonymize = nAnonymizeAstrumAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeAstrumAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeAstrumAmount));
        strAnonymizeAstrumAmount = strAnonymizeAstrumAmount.remove(strAnonymizeAstrumAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeAstrumAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
