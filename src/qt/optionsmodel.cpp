...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeAstrumAmount"))
        settings.setValue("nAnonymizeAstrumAmount", 1000);
    nAnonymizeAstrumAmount = settings.value("nAnonymizeAstrumAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeAstrumAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeAstrumAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeAstrumAmount:
            return QVariant(nAnonymizeAstrumAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeAstrumAmount:
            nAnonymizeAstrumAmount = value.toInt();
            settings.setValue("nAnonymizeAstrumAmount", nAnonymizeAstrumAmount);
            emit AnonymizeAstrumAmountChanged(nAnonymizeAstrumAmount);
            break;
        default:
            break;
        }
